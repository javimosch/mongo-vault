...
createApp({
  setup() {
    ...
    async function loadDashboard() {
      dashLoading.value = (dashData.value.length === 0);
      try {
        ...
        } else if (!anyRunning && dashboardTimer) {
    ...
    async function triggerBackup(targetId) {
      try {
        ...
        const json = await r.json();
        if (!r.ok) throw new Error(json.error || 'Failed to start backup');
        showToast('Backup started');
        setTimeout(loadDashboard, 1500);
      } catch (e) {
        showToast(e.message, false);
        // Refresh dashboard to show error status if recorded
        setTimeout(loadDashboard, 1500);
      }
    }
